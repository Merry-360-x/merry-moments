  ...
  return (
    <AnimatePresence>
      {isOpen && !hasBlockingDialog && (
        ...
        >
          <div className="relative rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_20px_50px_rgba(15,23,42,0.12)]">
            <button
              type="button"
              onClick={closePopup}
              aria-label="Close mobile money popup"
              className="absolute right-3 top-3 rounded-md px-2 py-1 text-xs text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700"
            >
              Close
            </button>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <Button size="sm" variant="outline" className="h-8 border-slate-300 px-3 text-xs text-slate-700" onClick={closePopup}>
                Maybe later
              </Button>
            </div>
          </div>
